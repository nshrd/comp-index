"""
CBMA14 Calculator - расчет индекса с применением 14-периодной скользящей средней
"""
import json
import logging
import sys
...
class CBMA14Calculator:
    """Калькулятор для расчета CBMA14 индекса"""
    ...
    def process_data(self, use_finance: bool = False, ma_period: int = 14) -> List[Dict]:
        ...
        Returns:
            Список обработанных данных с CBMA14
        """
        ...
        for i, ma_value in enumerate(ma_values):
            ...
            if original_index < len(sorted_items):
                ...
                result.append({
                    ...
                    'cbma14': round(ma_value, 2)
                })
        
        logger.info(f"Calculated CBMA14 for {len(result)} data points")
        self._processed_data = result
        return result
    
    def get_cbma14_history(self, from_timestamp: int = 0, to_timestamp: int = None, ma_period: int = 14) -> List[Dict]:
        """
        Получить историю CBMA14 в заданном временном диапазоне
        ...
        Returns:
            Отфильтрованные данные CBMA14
        """
        if self._processed_data is None or getattr(self, '_last_ma_period', None) != ma_period:
    ...
    def get_latest_cbma14(self) -> Optional[Dict]:
    ...
    def get_statistics(self) -> Dict:
        ...
        cbma14_values = [item['cbma14'] for item in self._processed_data]
        ...
        return {
            ...
            'cbma14': {
                'min': min(cbma14_values),
                'max': max(cbma14_values),
                'avg': sum(cbma14_values) / len(cbma14_values),
                'latest': cbma14_values[-1]
            },
            'original': {
    ...
    def export_to_json(self, output_file: Path) -> bool:
        ...
        try:
            ...
            logger.info(f"Exported {len(udf_format)} CBMA14 data points to {output_file}")
            return True
...
def main():
    ...
    # Создаем калькулятор
    calculator = CBMA14Calculator(data_file)
    ...
    print(f"  CBMA14 range: {stats['cbma14']['min']:.2f} - {stats['cbma14']['max']:.2f}")
    print(f"  Latest CBMA14: {stats['cbma14']['latest']:.2f}")
    
    # Показываем последние 5 значений
    print("\n📈 Latest 5 CBMA14 values:")
    for item in processed_data[-5:]:
        print(f"  {item['date']}: {item['original_value']} → {item['cbma14']}")
    
    # Экспортируем в CBMA14.json
    output_file = Path(__file__).parent.parent.parent / "data" / "CBMA14.json"
